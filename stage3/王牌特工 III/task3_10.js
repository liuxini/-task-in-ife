...
var main = (function(){
    ...
    var move = function(moverow,movecol) {
        var timer, num, i = 0;
        if ( path.length !==0 ) {
            return ; 
        }
        
        path = findway(map, map[hero.coordinate.row][hero.coordinate.col], map[moverow][movecol]);

        if( path ) {        
            num = path[path.length - 1].row ;
    ...
    var gameOver = function() {
       ...
       context.fillText('得分: '+score, canvas.width * 0.5, canvas.height * 0.5 - 70);
    ...
    var animate = function() {
        ...
        context.fillStyle = "cornflowerblue";
        context.fillText('分数: ' + score , 50, 20);
        if( !isGameOver ) {
