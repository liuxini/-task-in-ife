...
Guard.prototype.reset = function() {
    ...
    for( var i=0,len = this.guards.length; i<len; i++ ) {
        if( this.guards[i].row < main.row/2) {
            main.map[this.guards[i].row][this.guards[i].col].name = "null";
            this.guards.slice(i,1);
        } else {
            this.guards[i].row -= Math.floor(main.row/2);
            this.guards[i].y = this.guards[i].row * main.cellHeight;
        }
    }
